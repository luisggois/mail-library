...
NTLM_TYPE1_FLAGS = (NTLM_NegotiateUnicode | NTLM_NegotiateOEM | NTLM_RequestTarget | NTLM_NegotiateNTLM |
                    NTLM_NegotiateOemDomainSupplied | NTLM_NegotiateOemWorkstationSupplied | NTLM_NegotiateAlwaysSign |
                    NTLM_NegotiateExtendedSecurity | NTLM_NegotiateVersion | NTLM_Negotiate128 | NTLM_Negotiate56)

# we send these flags with our type 2 message
NTLM_TYPE2_FLAGS = (NTLM_NegotiateUnicode | NTLM_RequestTarget | NTLM_NegotiateNTLM | NTLM_NegotiateAlwaysSign |
                    NTLM_NegotiateExtendedSecurity | NTLM_NegotiateTargetInfo | NTLM_NegotiateVersion |
...
def create_NTLM_NEGOTIATE_MESSAGE(user, type1_flags=NTLM_TYPE1_FLAGS):
    ...
    message = protocol + stype + flags + DomainNameLen + DomainNameMaxLen + DomainNameBufferOffset + WorkstationLen + \
              WorkstationMaxLen + WorkstationBufferOffset + ProductMajorVersion + ProductMinorVersion + ProductBuild + \
              VersionReserved1 + VersionReserved2 + VersionReserved3 + NTLMRevisionCurrent

    assert BODY_LENGTH == len(message), "BODY_LENGTH: %d != msg1: %d" % (BODY_LENGTH, len(message))

    message += Workstation + DomainName
...
def create_NTLM_AUTHENTICATE_MESSAGE(nonce, user, domain, password, NegotiateFlags):
    ...
    msg3 = Signature + MessageType + LmChallengeResponseLen + LmChallengeResponseMaxLen + LmChallengeResponseOffset + \
           NtChallengeResponseLen + NtChallengeResponseMaxLen + NtChallengeResponseOffset + DomainNameLen + \
           DomainNameMaxLen + DomainNameOffset + UserNameLen + UserNameMaxLen + UserNameOffset + WorkstationLen + \
           WorkstationMaxLen + WorkstationOffset + EncryptedRandomSessionKeyLen + EncryptedRandomSessionKeyMaxLen +\
           EncryptedRandomSessionKeyOffset + NegotiateFlags + ProductMajorVersion + ProductMinorVersion +\
           ProductBuild + VersionReserved1 + VersionReserved2 + VersionReserved3 + NTLMRevisionCurrent
...
def calc_resp(password_hash, server_challenge):
    ...
    :param password_hash: 16-byte password hash
    :param server_challenge: 8-byte challenge from Type-2 message
    :return: 24-byte buffer to contain the LM response upon return
    """
